...
class UNet(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, t: torch.Tensor, severity: torch.Tensor
    ) -> torch.Tensor:
        """
        Forward pass
...
class DiffusionModel:
    ...
    def __init__(
        ...
        self.num_severity_levels = (
            num_severity_levels or diffusion_config.num_severity_levels
        )
    ...
    def p_sample(self, x: torch.Tensor, t: int, severity: torch.Tensor) -> torch.Tensor:
        ...
        mean = self.sqrt_recip_alpha[t] * (
            x - beta / self.sqrt_one_minus_alpha_cumprod[t] * predicted_noise
        )
    ...
    def training_step(
        self, images: torch.Tensor, severity: torch.Tensor
    ) -> torch.Tensor:
        """
...
class HealingDiffusion:
    ...
    def generate_for_severity(
        self, target_severity: int, num_images: int = 1
    ) -> List[GeneratedImage]:
        """
        ...
        severity = torch.full(
            (num_images,), target_severity, device=self.device, dtype=torch.long
        )
    ...
    def generate_trajectory(
        ...
        severity_range = np.linspace(start_severity, end_severity, num_steps).astype(
            int
        )
        ...
        for severity in severity_range:
            severity_tensor = torch.tensor(
                [severity], device=self.device, dtype=torch.long
            )
