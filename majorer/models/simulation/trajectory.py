...
from config import DEVICE, diffusion_config

from .diffusion_model import GeneratedImage, HealingDiffusion
...
class TrajectoryGenerator:
    ...
    def __init__(self, weights_path: Optional[Path] = None, device=None):
    ...
    def generate_trajectory(
        ...
        rate_multiplier = {"optimal": 0.7, "standard": 1.0, "suboptimal": 1.5}.get(
            treatment_scenario, 1.0
        )

        adjusted_rates = {
            k: int(v * rate_multiplier) for k, v in self.severity_healing_rates.items()
        }
        ...
        while day < num_days and current_severity > 0:
            ...
            intermediate_days = np.linspace(
                day, day + days_to_improve, num_intermediate + 1
            )[1:]
    ...
    def _interpolate_tissue(
        self, tissue_a: Dict[str, float], tissue_b: Dict[str, float], progress: float
    ) -> Dict[str, float]:
    ...
    def create_comparison_grid(
        self, frames: List[TrajectoryFrame], num_cols: int = 5
    ) -> np.ndarray:
        """
        ...
        for i, frame in enumerate(selected):
            ...
            cv2.putText(
                grid,
                frame.severity_name,
                (x_start + 10, target_h + 45),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.4,
                self._get_severity_color(frame.severity_level),
                1,
            )

        return grid

    def save_trajectory(self, frames: List[TrajectoryFrame], output_dir: Path):
    ...
    for day in range(0, 30, 5):
        ...
        frames.append(
            TrajectoryFrame(
                ...
                severity_name=["healed", "mild", "moderate", "severe", "critical"][
                    severity
                ],
