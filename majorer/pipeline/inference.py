...
from models.classification.classifier import ClassificationResult, WoundClassifier
...
from models.detection.detector import DetectionResult, WoundDetector
        ...
        return {
            ...
            "classification": {
                ...
                "wound_type_confidence": (
                    self.classification.wound_type_confidence if self.classification else None
                ),
                "severity": (self.classification.severity if self.classification else None),
                "severity_confidence": (
                    self.classification.severity_confidence if self.classification else None
                ),
            },
            "risk": {
...
class InferencePipeline:
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            logging.exception("Error loading pipeline: %s", e)
            return False

    def analyze(self, image: np.ndarray, skip_detection: bool = False) -> PipelineResult:
        ...
        if self.use_detection and not skip_detection:
            ...
            if detections:
                ...
                result.roi_image, result.roi_bbox = self._detector.extract_roi(image, detections[0])
            else:
        ...
        if self.use_segmentation:
            t0 = time.time()
            result.segmentation = self._segmentor.segment(result.roi_image, return_visualization=True)
            result.inference_times["segmentation"] = time.time() - t0

        if self.use_depth:
            t0 = time.time()
            mask = result.segmentation.mask if result.segmentation else None
            result.depth = self._depth_estimator.estimate(result.roi_image, mask)

            if result.segmentation:
                result.volume = self._volume_calculator.calculate(
                    result.depth.depth_normalized, result.segmentation.mask
                )
    ...
    def create_report_image(self, result: PipelineResult) -> np.ndarray:
        ...
        def add_text(text, color=(0, 0, 0)):
            nonlocal y
            cv2.putText(report, text, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
            y += line_height
    ...
    def batch_analyze(
        self, images: List[np.ndarray], output_dir: Optional[Path] = None
    ) -> List[PipelineResult]:
        results = []

        for i, image in enumerate(images):
            logging.info("Processing image %d/%d...", i + 1, len(images))
